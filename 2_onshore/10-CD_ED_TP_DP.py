...
"""
Generate monthly and annual onshore wind resource and potential products from hourly gross CF.
...
SCENARIOS_CFG = {
    "Dense": {"kx": 9.0, "ky": 6.0, "eff_array": 0.85},
    "Moderate": {"kx": 11.0, "ky": 8.0, "eff_array": 0.90},
    "Sparse": {"kx": 14.0, "ky": 10.0, "eff_array": 0.95},
}
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate monthly and annual onshore wind potential products from hourly gross CF."
    )
    parser.add_argument("--cf-dir", required=True, type=Path)
    parser.add_argument("--output-dir", required=True, type=Path)
    parser.add_argument("--year", required=True, type=int)
    parser.add_argument("--region", default="land", choices=["land", "onshore"])
    parser.add_argument("--cf-pattern", default="*.nc")
    parser.add_argument("--cf-var", default="CF")
    parser.add_argument("--rated-power-mw", default=5.25, type=float)
    parser.add_argument("--rotor-diameter-m", default=135.0, type=float)
    parser.add_argument("--availability-efficiency", default=0.97, type=float)
    parser.add_argument("--electrical-efficiency", default=0.98, type=float)
    return parser.parse_args()


def normalized_region_name(region):
    return "land" if region == "onshore" else region
